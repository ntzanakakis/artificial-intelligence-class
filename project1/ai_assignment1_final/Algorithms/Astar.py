...
class SequentialSearch(SearchBaseClass, ABC):
    ...
    def evaluation_function(self, node_current, weight, alt_flag):
        """
        f(x) = g(x) + h(x)
        """
    ...
    def execute_search(self, time_pause, weight,alt_flag=0) -> Tuple[Union[None, List[List[State]]], Union[None, List[MotionPrimitive]], Any]:
        ...
        open_set = []
        closed_set = []
        open_set.append(node_initial)
        total_nodes = 0                                                                    
        heuristic = self.heuristic_function(node_initial, alt_flag)
        while len(open_set)>0: #while we have nodes to visit
            cost = math.inf
            for node in open_set: 
                if self.evaluation_function(node,weight , alt_flag)<cost:
                    cost = self.evaluation_function(node,weight,alt_flag)
                    node_current = node

            open_set.remove(node_current)
            ...
            for primitive_successor in next_set:
                ...
                total_nodes += 1
                if collision_flag: #if collides, skip
                    continue
                if self.goal_reached(primitive_successor, node_current):
                    ...
                    break
                    #final = [final_path , heuristic , final_cost , total_nodes, True]
                for closed_child in closed_set: 
                    if child == closed_child and self.evaluation_function(open_child,weight,alt_flag)<self.evaluation_function(child,weight , alt_flag): #if it's been closed, skip
                        continue

                for open_child in open_set:
                    if child == open_child and self.evaluation_function(open_child,weight,alt_flag)<self.evaluation_function(child,weight , alt_flag):
                        continue
